...
[
  ...
  comida-transportada
  vuelta-nido?
  tiempo-recolectando    ; Tiempo que la hormiga lleva buscando comida desde la última recolección a un nido.
]
...
to accionar
  ...
  if not any? patches with [pcolor = green] and not any? obreras with [comida-transportada > 0]  ; Si no queda comida, fin.
  [stop]
  tick
end
...
to crear-nidos
  ...
  set id 1
  create-ordered-nidos max-nidos
  [
    set shape "house"
    set color yellow
    set size 1
...
to mover-obreras
  ;; Realizar el movimiento de las hormigas.

  ask obreras                                                      ; Acciones de hormiga
  [
    ifelse vuelta-nido?
    [mover-obrera-a-nido obtener-nido-cercano]
    [
      recolectar-comida
...
to recolectar-comida
  ...
  ifelse azar [right random max-giro] [left random max-giro]
  forward random max-avance

  if pcolor = green
  [
    set pcolor black
    set comida-encontrada comida-encontrada + 1
    set comida-transportada comida-transportada + 1
    cambiar-color-hormiga
  ]
end


to mover-obrera-a-nido [nido-cercano]
  ;; Mover hormiga obrera al nido pasado por argumento para almacenar comida

  face nido-cercano
  forward random max-avance
  if not ([id-nido] of nido-cercano = [id-nido-patch] of patch-here) [stop]

  if comida-transportada > 0   ; Si lleva comida, la almacena en el nido al que ha llegado
  [
    ask nido-cercano [set comida-almacenada comida-almacenada + ([comida-transportada] of myself)]
    set comida-transportada 0
  ]
  set vuelta-nido? false
end
